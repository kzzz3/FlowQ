...
TEST_CASE("interop_runner runs individual test") {
    flowq::quic::interop::interop_config config{};
    config.peer.name = "ngtcp2";
    config.peer.available = false;
    ...
    CHECK(result.status == flowq::quic::interop::test_status::skip);
...
TEST_CASE("interop_runner format_suite_result") {
    ...
    result.results.push_back({"loss_recovery", flowq::quic::interop::test_status::skip, "Not implemented", 0});
