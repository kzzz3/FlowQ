...
Current Windows evidence: `ctest --test-dir build/windows-msvc-vcpkg -C Debug --timeout 10 --output-on-failure` passes 519 CTest tests on the Windows MSVC/vcpkg preset. `scripts/validate-build.ps1 -Preset windows-msvc-vcpkg -BuildType Debug` validates configure, build, install, and package-consumer configure/build/run on the same preset. Strict warning evidence is tracked through `FLOWQ_ENABLE_STRICT_WARNINGS=ON` Debug builds with MSVC `/W4 /WX /permissive- /EHsc` plus `ctest --test-dir build/windows-msvc-vcpkg-warnings -C Debug --timeout 10 --output-on-failure`; refresh this evidence after test-count changes before production-candidate sign-off.
