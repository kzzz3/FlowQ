...
- Windows MSVC/vcpkg OpenSSL interop: 528/528 tests passing (`ctest --preset windows-msvc-vcpkg-interop-openssl --timeout 60 --output-on-failure`)
...
- aioquic runner: `scripts/run-aioquic-interop.ps1 -CondaEnv expr -Scenario all`, fail-closed on missing client binary, conda environment, aioquic package, or unsupported scenario; Windows runner behavior is covered by `flowq.aioquic_runner_script`; the runner passes `FLOWQ_CLIENT`, `FLOWQ_INTEROP_SCENARIO`, `FLOWQ_QUIC_PEER_HOST`, `FLOWQ_QUIC_PEER_PORT`, `FLOWQ_QUIC_STREAM_PAYLOAD`, and `FLOWQ_QUIC_EXPECT_ECHO` explicitly and records those client settings in JSON metadata
- Machine evidence validation: `scripts/validate-interop-evidence.py --results-dir docs/interop/results --min-full-flow-peers 1`
- Completed external peer reports can be imported without rerunning peers through `scripts/import-interop-evidence.py`; the importer reuses the validator and is covered by `flowq.interop_evidence_importer`
- The vcpkg-backed interop preset enables the `interop` manifest feature so ngtcp2 is installed for the optional Initial packet generation smoke target; this smoke remains outside the current recorded full-flow gate evidence
