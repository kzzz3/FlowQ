...
- Windows MSVC/vcpkg OpenSSL interop: 526/526 tests passing (`ctest --preset windows-msvc-vcpkg-interop-openssl --timeout 60 --output-on-failure`)
...
- CTest coverage for the checked-in interop evidence validator, FlowQ interop client runtime configuration, and Windows aioquic runner fail-closed behavior
