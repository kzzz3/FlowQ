...
- Windows MSVC/vcpkg OpenSSL interop: 525/525 tests passing (`ctest --preset windows-msvc-vcpkg-interop-openssl --timeout 60 --output-on-failure`)
...
- CTest coverage for the checked-in interop evidence validator and Windows aioquic runner fail-closed behavior
...
- aioquic runner: `scripts/run-aioquic-interop.ps1 -CondaEnv expr -Scenario all`, fail-closed on missing client binary, conda environment, aioquic package, or unsupported scenario; Windows runner behavior is covered by `flowq.aioquic_runner_script`
