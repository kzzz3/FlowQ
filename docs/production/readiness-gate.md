...
- Windows MSVC/vcpkg OpenSSL interop: 527/527 tests passing (`ctest --preset windows-msvc-vcpkg-interop-openssl --timeout 60 --output-on-failure`)
...
- CTest coverage for the checked-in interop evidence validator, FlowQ interop client runtime configuration, vcpkg-backed interop preset configuration, and Windows aioquic runner fail-closed behavior
