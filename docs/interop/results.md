...
- **Latest recheck**: 2026-05-31 on branch `codex-production-quic-hardening` after the interop runner was moved out of installed public headers; rebuilt `flowq_quic_client.exe` from the OpenSSL interop tree in the local Visual Studio 18/MSVC 19.50 environment and ran in conda environment `expr`; `bidirectional_stream` and `loss_recovery` both passed against aioquic 1.3.0.
